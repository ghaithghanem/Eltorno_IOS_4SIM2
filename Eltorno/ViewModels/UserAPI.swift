...
func DataBody(user:User, media: [Media]?, boundary: String) -> Data {
        ...
        body.append("Content-Disposition: form-data; name=\"username\"\(lineBreak + lineBreak)")
        body.append("\(user.username! + lineBreak)")
...
class UserAPI{
    func CreationCompte(user:User, image :UIImage, callback: @escaping (Bool,User?)->Void){
        ...
        session.dataTask(with: request) { (data, response, error) in
            DispatchQueue.main.async {
                                if let data = data {
                                    ...
                                    do {
                                        let test = try decoder.decode(User.self, from: data)
                                        callback(true,test)
                                    }catch{
    ...
    func UpdateProfil(user:User, image :UIImage, callback: @escaping (Bool,String?)->Void){
            ...
            guard let mediaImage = Media(withImage: image, forKey: "profilePicture") else { return }
            guard let url = URL(string: "http://localhost:3000/"+UserDefaults.standard.string(forKey: "_id")!) else { return }
            var request = URLRequest(url: url)
